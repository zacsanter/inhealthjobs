...
function displayResponse(response) {
    // Check if the response message contains the specific text
    if (response.message && response.message.includes('Here are my top 3 recommendations for states to practice')) {
        // Get the elements
        var locationContainer = document.getElementById('location-container');
        var chatWindow = document.getElementById('chat-window');

        // Make the location container visible
        locationContainer.style.display = 'block'; // Adjust display style as needed

        // Append the location container to the chat window
        chatWindow.appendChild(locationContainer);
    }
